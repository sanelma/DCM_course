...
list.of.packages <- c("foreign","fExtremes","plyr","stats","kulife","survival",#"devtools",
                      "reshape","reshape2","gtools","psych","tibble","AlgDesign",
                      ...
                      "ggplot2","fastDummies","randomForest","stringr")
...
if(grepl("eu",Sys.info()["nodename"]) & Sys.info()["user"] =="schmidb"){ # Schmid's ETH Euler supercluster account
  parentpath <- paste0("/cluster/home/schmidb/r-input/002_sp_exercise/design_test/")
  cores <- 24
}else if(Sys.info()["nodename"]=="IVT-THKPD-30"){ # Schmid's ETH Laptop
  parentpath <- "I:/Lehre/MSc Messung und Modellierung/2021/005_estimation/"
  cores <- 1
}else if(Sys.info()["nodename"]=="IVT-ESP-27"){ # Schmid's ETH Desktop
  parentpath <- "I:/Lehre/MSc Messung und Modellierung/2021/002_sp_exercise/design_test/"
  cores <- 1
}else if(grepl("staff-net-vpn",Sys.info()["nodename"]) & Sys.info()["user"] =="schmidb"){ # Schmid's personal Macbook
  if(grepl("/Users/schmidb",getwd()) ){
    parentpath <- "/Users/schmidb/Desktop/002_sp_exercise/design_test/" # Schmid's laptop desktop
  }else{
    parentpath <- "/Volumes/baug_ivt_vpl_1/Lehre/MSc Messung und Modellierung/2021/002_sp_exercise/design_test/" # Schmid's working directory on I:
  }
  cores <- 2
}else if(grepl("staff-net-vpn",Sys.info()["nodename"]) & Sys.info()["user"] =="bas"){ # Schmid's personal iMac
  if(grepl("/Users/schmidb",getwd()) ){
    parentpath <- "/Users/schmidb/Desktop/002_sp_exercise/design_test/" # Schmid's iMac desktop
  }else{
    parentpath <- "/Volumes/baug_ivt_vpl_1/Lehre/MSc Messung und Modellierung/2021/002_sp_exercise/design_test/" # Schmid's working directory on I:
  }
  cores <- 4
}else if(grepl("staff-net-vpn",Sys.info()["nodename"]) & Sys.info()["user"] =="bnair"){ # Other personal iMac
    parentpath <- "/Volumes/baug_ivt_vpl_1/Lehre/MSc Messung und Modellierung/2021/002_sp_exercise/design_test/" # Working directory on I:
  cores <- 2
}
